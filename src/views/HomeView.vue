    ...
    <div class="flex-1 flex flex-col p-8 md:p-12 max-w-6xl mx-auto w-full overflow-hidden">
      ...
      <div class="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-12 min-h-0">
        ...
        <div v-if="isElectron" class="lg:col-span-2 flex flex-col min-h-0">
          ...
          <div v-if="uiStore.recentFiles.length > 0" class="flex-1 overflow-y-auto space-y-2">
            <div 
              v-for="file in uiStore.recentFiles" 
              :key="file.name + file.path"
              class="group flex items-center justify-between p-3 border border-gray-100 dark:border-[#333333] rounded-lg hover:border-blue-500/50 transition-all cursor-pointer"
              @click="handleRecentClick(file)"
            >
              <div class="flex items-center gap-4">
                <div 
                  class="w-10 h-10 rounded-main bg-app-hover flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors"
                >
          ...
          <div v-else class="flex flex-col items-center justify-center py-12 border-2 border-dashed border-divider rounded-main">
            <EmptyState icon="fa-file-circle-xmark" size="sm" subtitle="暂无最近打开的项目" />
          </div>
        </div>
        ...
        <div v-else class="lg:col-span-2 flex flex-col min-h-0">
          ...
          <div class="flex-1 overflow-y-auto space-y-2">
            ...
            >
              <div class="flex items-center gap-4">
                <div class="w-10 h-10 rounded-main bg-app-hover flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-colors">
              ...
              <div class="opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-1 group-hover:translate-x-0">
                <div class="text-xs text-blue-600 font-bold flex items-center gap-1">
                  立即下载 <i class="fa-solid fa-chevron-right text-[10px]"></i>
                </div>
              </div>
            </div>

            <div 
              v-for="item in DESCRIPTIVE_LINKS"
              :key="item.title"
            ...
            >
              <div class="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div class="text-ui-detail">{{ item.desc }}</div>
                </div>
              </div>
              <div class="opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-1 group-hover:translate-x-0">
                <i class="fa-solid fa-up-right-from-square text-[10px] text-gray-300"></i>
              </div>
            </div>
          </div>
        </div>
